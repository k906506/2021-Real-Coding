...
public class MockServiceTest {
    ...
    @Test
    public void 어떠한_챔피언이름_입력해도_무조건베인을_리턴한다() {
        Champion champion = mock(Champion.class);
    ...
    // 1. when, thenReturn을 사용하여 어떠한 라인을 입력해도 Mid를 리턴하도록 테스트하세요
    @Test
    public void 어떠한_라인을_입력해도_무조건미드를_리턴한다() {
        Champion champion = mock(Champion.class);
        champion.setPosition("Support");

        //champion.getName()을 호출하면 "Mid"를 리턴한다.
        when(champion.getPosition()).thenReturn("Mid");
        assertThat(champion.getPosition(), is("Mid"));
    }


    // 2. 챔피언 이름으로 야스오를 저장하면, doThrow를 사용하여 Exception이 발생하도록 테스트 하세요.
    @Test(expected = IllegalArgumentException.class)
    public void showDoThorwWhenYasuo(){
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void veritySetPositionMid(){
        Champion champion = mock(Champion.class);
        champion.setName("조이");
    ...
    public void shouldOneTimeRunForChampionArraySize(){
        ...
        topChamp.setName("사일러스");
        topChamp.setPosition("미드");
        topChamp.setHasSkinCount(2);
    ...
    @Test
    public void shouldPassWhenOneRun200ms(){
        Champion champion = mock(Champion.class);
        champion.setName("티모");
        champion.setPosition("탑");
        champion.setHasSkinCount(3);
    ...
    @Test
    public void shouldCheckLisandraNameAndPosition(){
    ...
    @Test
    public void shouldCheckAllChampionName(){
        List<Champion> championList = new ArrayList<>();
        championList.add(new Champion("루시안", "바텀", 5));
        championList.add(new Champion("사일러스", "서폿", 5));
        championList.add(new Champion("리신", "탑", 5));
    ...
    @Test
    public void 가짜챔피언리턴후검증(){
        when(mockService.findByName(any(String.class))).thenReturn(new Champion("사일러스", "미드", 2));
        assertThat(mockService.findByName("사일러스").getHasSkinCount(), is(2));
        verify(mockRepository, times(1)).findByName(anyString());
    }

    // 4. 2개 이상의 가짜 챔피언 객체를 List로 만들어 리턴하고, 하나씩 해당 객체를 검색한 뒤 검색을 위해 호출한 횟수를 검증하세요.
    @Test
    public void 가짜챔피언리스트리턴후검증(){
        List<Champion> championList = new ArrayList<>();
        championList.add(new Champion("루시안", "바텀", 5));
        championList.add(new Champion("사일러스", "서폿", 5));
        ...
        assertThat(mockService.findAllChampions().get(0).getHasSkinCount(),is(5));
